  ...
  return (
    <aside className="h-full w-full absolute top-0 z-10 backdrop-blur-sm -left-full aside">
      <div className="bg-[#191919] h-full w-80 -left-80 relative transition-all duration-300 ease-in menu-aside border-r">
        <button id="close" className="absolute top-0 mt-2 right-2 z-10 hover:scale-110 trasition">
        ...
        <div className="flex items-center justify-start px-2 py-5 gap-2">
          ...
          <span className="text-purple-500 text-xl font-bold">{session?.user?.name}</span>
        </div>
        <ul className="text-lg text-white flex flex-col gap-3">
          <Link href={`/app/${session?.user?.name}/resume`} className="bg-transparent">
            <li className="p-2 hover:pl-4 hover:bg-primary hover:text-yellow-500 transition-all border-gray-500">
              <ActivityIcon />
              <span>Resumen</span>
            </li>
          </Link>
          <Link href={`/app/${session?.user?.name}/accounts`} className="bg-transparent">
            <li className="p-2 hover:pl-4 hover:bg-primary hover:text-yellow-500 transition-all border-gray-500">
              <CoinIcon />
              <span>Cuentas</span>
            </li>
          </Link>
          <Link href={`/app/${session?.user?.name}/debts`} className="bg-transparent">
            <li className="p-2 hover:pl-4 hover:bg-primary hover:text-yellow-500 transition-all border-gray-500">
          ...
          <Link href="#" className="bg-transparent cursor-not-allowed">
            <li className="p-2 hover:pl-4 hover:bg-primary hover:text-yellow-500 transition-all border-gray-500">
              <ExchangeIcon />
              <span>
                Divisas ~ <span className="text-palette text-sm">Conversor</span>
              </span>
            </li>
          </Link>
        ...
        <h1 className="w-full text-center opacity-50 italic text-white my-10">Pronto se habilitarán las opciones</h1>
      </div>
    </aside>
  )
}
