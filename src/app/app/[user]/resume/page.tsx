...
export default function Resume() {
  ...
  const calcularCrecimiento = (cuenta: string) => {
    const ingresosDelMes = ingresosFiltrados
      .filter((ingreso) => ingreso.account === cuenta)
      .reduce((total, ingreso) => total + ingreso.value, 0);
    const gastosDelMes = gastosFiltrados
      .filter((gasto) => gasto.accountname === cuenta)
        ...
        <div className="mt-5">
          {expenses.length > 1 && (
            <small className="italic opacity-70">
              {obtenerConsejoFinanciero()}
            </small>
          )}
          <div className="grid grid-cols-2 items-center mt-4">
            <div className="flex gap-3 items-center">
              <h3 className="text-md md:text-2xl text-[var(--color-usage)] font-medium">
                {monthName} - {anioActual}
              </h3>
              <OpenButton />
            </div>
