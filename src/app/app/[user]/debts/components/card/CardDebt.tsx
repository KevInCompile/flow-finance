...
export default function CardDebt({ data, setData, fullData, setRefresh }: { data: Debt, setData: React.Dispatch<SetStateAction<Debt[]>>, fullData: Debt[], setRefresh: React.Dispatch<boolean> }) {
  ...
  const handleDelete = async (id: number, idDebt: number, value: number) => {
    ...
    if(isDelete){
      const filter = { ...data, totaldue: data.totaldue + value,  payments: data.payments.filter(item => item.id !== id) }
            ...
            }
            {
              payments?.map((pay: any, i) => (
                <article className="text-white grid grid-cols-2 py-1.5 border-b border-gray-500" key={pay.id}>
                  <span><b className="text-palette">{i + 1}.</b> $
                    {pay.payvalue.toLocaleString()}
                  </span>
                  <span className="text-end text-green-500 font-medium flex justify-end gap-2 text-sm">
                    {pay?.paymenttype}
                    <DeleteConfirmation deleteItem={() => handleDelete(pay.id, pay.debtsid, pay.payvalue)} message="Quiere eliminar el pago?"/>
                  </span>
                </article>
              ))
            }
